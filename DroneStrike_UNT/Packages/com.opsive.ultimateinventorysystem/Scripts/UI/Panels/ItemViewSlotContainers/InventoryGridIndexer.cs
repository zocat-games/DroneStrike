    ...
    public class InventoryGridIndexer
    {
        ...
        public void RemoveEmptyIndexes()
        {
            ...
            foreach (var indexedItem in m_IndexedItems) {
                pooledOrderedItemInfos.EnsureSize(indexedItem.Value.index+1);

                pooledOrderedItemInfos[indexedItem.Value.index] = indexedItem.Value.itemInfo;
            }

            pooledOrderedItemInfos.RemoveAll(info => info.ItemStack == null);
            
            m_IndexedItems.Clear();
            for (var i = 0; i < pooledOrderedItemInfos.Count; i++) {
                var itemInfo = pooledOrderedItemInfos[i];
                m_IndexedItems[itemInfo.ItemStack] = (i, (ItemInfo)itemInfo.ItemStack);
            }
