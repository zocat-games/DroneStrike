namespace Opsive.UltimateInventorySystem.Editor.Utility.InventoryDatabaseImportExport
{
    ...
    public class AttributeImportExport
    {
        ...
        public virtual void SetAttributeValue(InventorySystemDatabase database, AttributeBase attribute, ImportAttributeInfo attributeInfo)
        {
            ...
            try
            {
                ...
                if (converter is ITypeConverterWithDatabase converterWithDatabase)
                {
                    converterWithDatabase.SetInventorySystemDatabase(database);
                }
                var result = converter.ConvertFromString(newValue as string);
                AttributeEditorUtility.SetOverrideValueAsObject(attribute, result);
            }
            catch
            {
                Debug.LogWarning($"The attribute '{attribute.Name}' cannot import the value '{newValue}' with type '<{valueType.FullName}>'");
            }
            ...
            m_AttributeImportExport.IterateThroughAttributes(row, (attributeInfo) =>
            {
                ...
                if (attribute != null) {
                    ...
                    if (attributeInfo.Value != null) {
                        m_AttributeImportExport.SetAttributeValue(database,attribute, attributeInfo);
                    }
                    ...
                    m_AttributeImportExport.SetAttributeValue(database,newAttribute, attributeInfo);
                }
            });
    ...
    public class ItemDefinitionImportExport : DataTableImportExport<ItemDefinition>
    {
        ...
        protected override void ImportObjectData(string objName, DataRow row, InventorySystemDatabase database)
        {
            ...
            if (columns.Contains(CATEGORY_KEY)) {
                var categoryName = row[CATEGORY_KEY] as string;
                if (string.IsNullOrWhiteSpace(categoryName) == false) {
                    var category = database.Get<ItemCategory>(categoryName);
                    if (category == null) {
                        Debug.LogWarning($"The Item Definition '{objName}' has an Item Category '{categoryName}' which does not exist in the database, it will be ignored.");
                    } else {
                        definition.SetCategory(category, Relation.None);
                    }
                }
            }
            ...
            if (columns.Contains(PARENT_KEY)) {
                ...
                if (string.IsNullOrWhiteSpace(parentName) == false) {
                    ...
                    if (parentDefinition == null) {
                        Debug.LogWarning($"The Item Definition '{objName}' has a parent '{parentName}' which does not exist in the database, it will be ignored.");
                    } else {
