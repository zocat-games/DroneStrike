...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.FirstPersonController.Character
{
    ...
    public class FirstPersonObjectsInspector : Shared.Editor.Inspectors.UIStateBehaviorInspector
    {
        ...
        protected override void ShowHeaderElements(VisualElement container)
        {
            ...
            var lookOffsetContainer = new VisualElement();
            FieldInspectorView.AddField(target, target, "m_IgnorePositionalLookOffset", container, (object obj) =>
            {
                ...
                lookOffsetContainer.style.overflow = (target as FirstPersonObjects).IgnorePositionalLookOffset ? Overflow.Visible : Overflow.Hidden;
            });
            container.Add(lookOffsetContainer);
            FieldInspectorView.AddField(target, target, "m_PositionOffset", lookOffsetContainer);
            FieldInspectorView.AddField(target, target, "m_MoveSpeed", lookOffsetContainer);
            lookOffsetContainer.style.overflow = (target as FirstPersonObjects).IgnorePositionalLookOffset ? Overflow.Visible : Overflow.Hidden;
        }
    }
}