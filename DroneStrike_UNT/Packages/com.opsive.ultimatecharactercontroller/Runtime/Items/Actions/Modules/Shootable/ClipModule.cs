...
namespace Opsive.UltimateCharacterController.Items.Actions.Modules.Shootable
{
    ...
    /// </summary>
    [Serializable]
    public abstract class ShootableClipModule : ShootableActionModule, IModuleReloadClip, IModuleItemDefinitionConsumer
    {
        ...
        /// <summary>
        /// The Item Definition of the ammo.
        /// </summary>
        public abstract ItemDefinitionBase ItemDefinition { get; set; }

        /// <summary>
        /// Is the clip full?
        /// </summary>
        /// <returns>True if the clip is full.</returns>
        ...
        public int GetItemDefinitionRemainingCount()
        {
            return ClipRemainingCount;
        }
        ...
        public void SetItemDefinitionRemainingCount(int count)
        {
            SetClipRemaining(count);
        }
    }
    ...
    public class SimpleClip : ShootableClipModule
    {
        ...
        public override ItemDefinitionBase ItemDefinition
        {
            ...
            set {
                Debug.LogError("Error: The SimpleClip module cannot set the ItemDefinition.");
            }
        }
        ...
        public override ShootableAmmoData GetAmmoDataInClip(int index)
        {
            ...
            if (!m_ClipRemaining[index].Valid) { return ShootableAmmoData.None; }
        ...
        public override void ReloadClip(bool fullClip)
        {
            ...
            if (mainAmmoModule.IsAmmoShared()) {
                ...
                if (m_TotalReloadAmount > m_TotalAmmoAmount) {
                    ...
                    var targetAmount = fullClip ? Mathf.CeilToInt(totalAmount / (float)activeSharedInstances) : clipRemainingCount + 1;
                    ...
                    reloadAmount = Mathf.Min(m_TotalAmmoAmount, (fullClip ? (m_ClipSize - clipRemainingCount) : 1));
                }
            } else {
                // The consumable ItemIdentifier doesn't share with any other objects.
                reloadAmount = Mathf.Min(remainingAmmo, (fullClip ? (m_ClipSize - clipRemainingCount) : 1));
            }
                ...
                for (int i = 0; i < Inventory.SlotCount; ++i) {
                    ...
                    if (item != null) {
                        ...
                        for (int j = 0; j < shootableWeapons.Length; ++j) {
                            ...
                            m_TotalReloadAmount += shootableWeapons[j].ClipSize - shootableWeapons[j].ClipRemainingCount;
